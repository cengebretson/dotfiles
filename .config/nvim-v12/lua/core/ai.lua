...
local function find_ai_pane()
	...
	if session == "" then return nil, nil, "Not inside tmux" end
	local panes = vim.fn.system(
		"tmux list-panes -s -t " .. vim.fn.shellescape(session) .. " -F '#{session_name}:#{window_index}.#{pane_index} #{pane_pid}'"
	)
		...
		if shell_pid then
			...
			for child_pid in children:gmatch("%d+") do
				...
				if args:match("claude") then
					claude_pane = pane_id
					break
				elseif args:match("codex") then
					codex_pane = pane_id
				end
			end
		end
		if claude_pane then break end
	end
	if claude_pane then return claude_pane, "claude", nil end
	if codex_pane then return codex_pane, "codex", nil end
...
local function tmux_send(pane_id, text)
	...
	f:close()
	vim.fn.system(
		"tmux load-buffer " .. vim.fn.shellescape(tmp)
		.. " && tmux paste-buffer -t " .. vim.fn.shellescape(pane_id)
		.. " && tmux send-keys -t " .. vim.fn.shellescape(pane_id) .. " Enter"
	)
	os.remove(tmp)
	if vim.g.claude_bell then
		local tty = vim.fn.system(
			"tmux display-message -t " .. vim.fn.shellescape(pane_id) .. " -p '#{pane_tty}'"
		):gsub("\n", "")
