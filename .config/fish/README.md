...
| `tmux_attention_claim` / `tmux_attention_disown` | Generated by `tmux-attention shell-init fish` in `config.fish`, not defined here. `claude`/`codex` call them to stamp the pane for one agent launch, so a marker left by a killed agent can be told from a live one. The `tmux-attention` CLI itself is a symlink on `PATH` into the tmux plugin |
